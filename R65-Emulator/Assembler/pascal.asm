...
* VERSION 5.8   WRITING TO CPNT STRINGS
* VERSION 6.0   SUPPORTS USING CHAIN IN SYSTEM
*
        TIT R65 PASCAL RUNTIME VERSION 6.0
*
...
EOPROG  BSS 2   END OF USER PROGRAM
...
DEVPNT  BSS 2   DEVICE CPNT POINTER
*
FILFLG  EQU $DA
FILERR  EQU $DB
FILDRV  EQU $DC
...
USEREND WRD $C7FF
*
* VECTOR TABLE, DO NOT MOVE OR RESHUFFLE
*
HARGUM  JMP $FCF2
*
VSTOP   JMP STOP
...
*************
*
QUIT    JSR PRTINF      NO, STOP PASCAL
        BYT 'Quit Pascal'+$80
        LDA SFLAG
        AND =$7E        CLEAR PASCAL RUNTIME BIT
        STA SFLAG       AND ESCAPE FLAG IN SFLAG
        LDX SAVS
        TXS
        JMP (VMON)
*
...
* NO ARGUMENTS
*
STOP    LDA RUNERR      IF RUNERR ALREADY SET
        CMP =$84        TO RUN/CHAIN ERROR
        BNE STOP0
*
        LDA FILDRV      AND FILDRV IS DISK 1
        CMP =1
        BNE STOP0
*
        LDA =0          TRY TO LOAD FROM DISK 0
        STA FILDRV
        STA RUNERR
        LDA STPROG
        LDX STPROG+1
...
***************************************
*
PRTCHR0 LDX DEVPNT+1
        BNE PRTCHR7     WRITE TO STRING
        LDX DEVICE
...
*
PRTCHR7 PHA             SAVE CHARACTER
        STY ARG3                TO PRESERVE Y
*
        LDY =255
PRTCHR8 INY
        CPY =62         FIND END MARK
        BCS PRTCHR9     MAX SIZE OF STRING
        LDA (DEVPNT),Y
        BNE PRTCHR8
*
        PLA             GET CHAR
        STA (DEVPNT),Y  CHARACTER TO CPNT
        LDA =0
        INY
        STA (DEVPNT),Y  END MARK TO CPNT
        INC DEVPNT
        BNE *+4
        INC DEVPNT+1
        LDY ARG3                RESTORE Y
        RTS
*
PRTCHR9 LDX =$91        STRING OVERFLOW
*
