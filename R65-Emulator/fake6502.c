...
static void jmp() {
    if (catchSubroutine(ea)) {
        value = pull16();   // execute a rts
        pc = value + 1;
    }
    else
        pc = ea;
...
void nmi6502() {
    printRegisters();
    push16(pc);
    status = status & (~FLAG_BREAK) & (~FLAG_INTERRUPT); // not a break, disable irq
    push8(status);
