...
begin
  ...
  end else if nlines<maxlines-1 then s:=_new
  else s:=nil;
  rnew:=s;
  if s<>nil then begin
...
proc showtop;
begin
  _intstr(line,stemp,3); putontop(stemp,5,true);
  _intstr(nlines-1,stemp,3); putontop(stemp,12,true);
end;
...
begin
  clrmessage;
  if not again then _strcpy(stemp,fs);
